	...
	return (
		<div className="shell-screen shell-screen--friends">
			...
			<div className="friends-panel">
				{!currentUser && ( <p className="form-error alert alert-danger">Login first to view your friends.</p> )}
				...
				{currentUser && (
					<>
								...
								<ul className="friends-list">
									{friendSearchResults.map((user) => (
										<li key={user.id} className="friends-item">
											...
											{getRelationStatus(user) === "none" && (<button className="btn btn-primary" type="button" onClick={() =>
												submitSearchFriend(user.id)} disabled={isDisabled}> add </button>)}
											{getRelationStatus(user) === "accept" && (<button className="btn btn-outline-success" type="button" onClick={() =>
												submitAcceptFriend(user.id)} disabled={isDisabled}> accept </button>)}
						...
						{directChat.directError && ( <p className="form-error alert alert-danger" role="alert">	{directChat.directError} </p>)}
							...
							<ul className="friends-list">
								{friendList.map(friend => (
									<li key={friend.id} className="friends-item">
										...
										<span className="friends-meta badge text-bg-info">#{friend.id}</span>
										<button className="btn btn-outline-primary" type="button" onClick={() => openDirectChat(friend)} >
											Message
										</button>
										{canInvite &&
											(<button
											...
											>
												{hasPendingInvitation(friend.id) ? 'invitation sent' : 'Invite'}
											</button>										)}
										<button
