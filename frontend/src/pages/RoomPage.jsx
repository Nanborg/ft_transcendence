...
export function RoomPage({ title, description, socket, currentUser, room, chat })
{
	...
	const pageActions = [ { label: 'Back to Lobby', href: '#/lobby' }, ];
	...
	if (currentRoom)
	{
		...
		{
			startButton = (
				<button type="button" className="room-reconnect-button btn btn-outline-warning"
				onClick={() => socket.emit('game:resync', { roomId: currentRoom.id })}
				disabled={isDisabled} style={{ backgroundColor: 'orange', color: 'white' }}>Reconnect</button>
			);
		}
	...
	return (
		<div className="shell-screen shell-screen--room">
			<PageHeading title={title} description={description} actions={pageActions} />
