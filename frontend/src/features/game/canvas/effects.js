...
export function drawShieldBreakEffects({ context, effects, camera, now })
{
	effects.forEach((effect, entityId) =>
	{
		...
		const screen = worldToScreen( { x: effect.posX, y: effect.posY }, camera );
		const tilePixels = camera.tileSize * camera.scale;
		...
		for (let index = 0; index < 8; index += 1)
		{
			...
			context.beginPath();
			context.moveTo( screen.x + Math.cos(angle) * innerRadius, screen.y + Math.sin(angle) * innerRadius );
			context.lineTo( screen.x + Math.cos(angle) * outerRadius, screen.y + Math.sin(angle) * outerRadius );
			context.stroke();
		}
		context.restore();
	});
