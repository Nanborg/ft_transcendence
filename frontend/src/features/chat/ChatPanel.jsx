...
export function ChatPanel({chat, disabled = false, compact = false, onInputFocusChange})
{
	...
	return (
		<section className={`room-chat${compact ? ' room-chat--compact' : ''}`} aria-label="Room chat">
			<h3>Chat</h3>
			{chatMessages.length === 0 && ( <p className="room-muted">No messages yet.</p> )}
			<ul ref={messageRef} className="room-chat-messages" aria-live="polite">
				{chatMessages.map(chatMessage =>
				(
					<li key={chatMessage.id ?? `${chatMessage.timestamp}.${chatMessage.author?.id}`}>
						{/* FALLBACK: Deleted users keep message visible. */}
			...
			{chatError && ( <p className="room-error alert alert-danger" role="alert"> {chatError} </p> )}
			<form className="room-chat-form" onSubmit={sendChatMessage}>
				...
				<button className="btn btn-primary" type="submit" disabled={disabled || !chatInput.trim()}>
					Send
				</button>
			</form>
		</section>
