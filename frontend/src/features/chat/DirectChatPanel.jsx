	...
	if (!selectedUser)
	{
		return (
			<div className="direct-chat-panel direct-chat-panel--compact">
				...
				{conversations.length === 0 && ( <p className="room-muted">	No private conversations yet. </p> )}
		...
		>
			<header className="direct-chat-header">
				<button
					type="button"
					className="btn btn-outline-secondary"
					onClick={() => { onInputFocusChange?.(false); closeConversation(); }}
			...
			<ul className="direct-chat-messages" aria-live="polite" >
				...
				{directMessages.map(message =>
				{
					...
					return (
						...
						>
							<strong> {isOwnMessage ? 'You' : message.author?.name || 'Deleted user'} </strong>
			...
			{directError && ( <p className="alert alert-danger" role="alert"> {directError} </p> )}

			<form className="direct-chat-form" onSubmit={sendDirectMessage} >
				<label className="visually-hidden" htmlFor="private-chat-message" >
					Private message
				</label>
				...
				<button
					...
					type="submit"
					disabled={ isSelectedUserBlocked || !directInput.trim() }
				>
					Send
				</button>
			</form>
