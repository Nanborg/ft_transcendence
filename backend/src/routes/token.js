...
class AuthRefreshError extends Error { //test-nico
...
function verifyRefreshToken(refreshToken) { //test-nico
	try {
...
router.post("/", async (req, res) => {
	...
	if (!refreshToken) {
		return res.status(401).json({ //test-nico
	...
	try {
		...
		const tokens = await prisma.$transaction(async (tx) => { //test-nico
			...
			const revoked = await tx.refreshToken.updateMany({ //test-nico
				where: {
					id: tokenRecord.id,
					isRevoked: false
				},
			...
			await tx.refreshToken.create({ //test-nico
				data: {
		...
		if (error instanceof AuthRefreshError) {
			return res.status(error.status).json({ //test-nico
				error: error.message,
				code: error.code
			});
		}
