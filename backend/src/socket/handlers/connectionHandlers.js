...
function registerConnectionHandlers(io, socket)
{
	socket.on('debug:latency:check', ({ clientSentAt }) =>
	{
		...
		socket.emit('debug:latency:result', { clientSentAt, backendReceivedAt: Date.now(), });
	});

	socket.on('disconnect', async () =>
	{
		try {
			const stopInput = { up: false, down: false, left: false, right: false, };
			...
			scheduleDisconnect( socket.user.id, socket.id, async () =>
			{
					const userRooms = await getRoomsByUserId(socket.user.id);
					if (!isOnline(socket.user.id))
						io.emit('user:status', { userId: socket.user.id, isOnline: false, });
					for (const room of userRooms)
					{
						try {
		...
		{
			socket.emit('room:error', { event: 'disconnect', message: error.message, });
		}
	});
}
